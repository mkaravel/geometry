...
int main(int argc, char** argv)
{
    ...
    //    std::cout << "max segment length: " << max_length << std::endl;
    ...
    for (std::size_t i = 0; i < n_red; ++i)
    {
        ...
        ifs >> x1 >> y1 >> x2 >> y2;
        linestring_type ls;
        ls.push_back( point_type(x1, y1) );
        ls.push_back( point_type(x2, y2) );
        mls.push_back( ls );
    }
    ...
    std::cout << "MLS size: " << boost::size(mls) << std::endl;
              ...
              << n_segs << std::endl << std::endl;
    {
        ...
        for (int i = 0; i < NUM_ITERATIONS; ++i)
        {
            n_segs = get_segments_via_iterator(mls);
        }
    }
